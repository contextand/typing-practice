    ...
    <div class="list-header">
      ...
      <div class="header-row">
        ...
        <div class="sort-group">
          <button class="sort-btn" data-key="date">정리한 날 <span></span></button>
          <button class="sort-btn" data-key="title">이름 <span></span></button>
        </div>
      </div>
    </div>
...
let sortKey = 'date';
const directions = { date: 'desc', title: 'asc' };
let keyword = '';
let selected = null;

function metaLine(book) {
  const date = book.organizedDate ? book.organizedDate.replace(/-/g, '.') : null;
  return [date, book.author, book.genre, book.publisher].filter(Boolean).join(' / ');
}
...
function visibleBooks() {
  const dir = directions[sortKey] === 'desc' ? -1 : 1;
  return BOOKS.filter(matches).sort((a, b) => {
    if (sortKey === 'title') return byTitle(a, b) * dir;
...
function renderList() {
  ...
  books.forEach(book => {
    ...
    meta.textContent = metaLine(book);
...
function renderSortButtons() {
  sortBtns.forEach(btn => {
    ...
    btn.querySelector('span').textContent = directions[key] === 'desc' ? '↓' : '↑';
  });
}
...
sortBtns.forEach(btn => {
  btn.addEventListener('click', () => {
    const key = btn.dataset.key;
    if (key === sortKey) {
      directions[key] = directions[key] === 'desc' ? 'asc' : 'desc';
    } else {
      sortKey = key;
    }
    renderSortButtons();
...
def main():
    raw_books = fetch_books_from_sanity()
    books = [b for b in raw_books if b.get("passages")]

    print(f"Fetched {len(books)} books from Sanity")
...
<style>
  ...
  * {{ box-sizing: border-box; margin: 0; padding: 0; }}
  ...
  @media (min-width: 1001px) {{
    ...
    /* Invisible 1px textarea captures keystrokes without covering the panel */
    .pc-input {{
      position: fixed;
      top: 0; left: 0;
...
const FONT_LINKS = {{
  ...
  'JoseonLogo': 'https://event.chosun.com/100/100font.html',
...
document.addEventListener('click', (e) => {{
  if (!isPc() || typePage.classList.contains('open') || e.target.closest('a')) return;
  // Don't steal focus while the user has text selected — focusing collapses it
...
window.addEventListener('focus', () => {{
  const sel = window.getSelection();
  if (isPc() && (!sel || sel.isCollapsed)) inputArea.focus();
}});
